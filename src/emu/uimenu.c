...
#define VISIBLE_GAMES_IN_LIST 9000
...
static void menu_speed_hacks(running_machine *machine, ui_menu *menu, void *parameter, void *state);
...
INLINE void toggle_none_default(input_seq *selected_seq, input_seq *original_seq, const input_seq *selected_defseq)
{
	...
	if (input_seq_get_1(original_seq) == SEQCODE_END)
	{
		...
		*selected_seq = *selected_defseq;
		/*
		for(int i=0; i<8;i++)
			selected_seq->code[i] = selected_defseq->code[i];
		*/
	}
...
static void ui_menu_draw(running_machine *machine, ui_menu *menu, int customonly)
{
	...
	if (visible_width + 2.0f * UI_BOX_LR_BORDER >= 0.99f)
		visible_width = 0.99f - 2.0f * UI_BOX_LR_BORDER;
	...
	for (typedesc = input_type_list(machine); typedesc != NULL; typedesc = typedesc->next)
		...
		if (typedesc->group == group && typedesc->name != NULL && typedesc->name[0] != 0)
		{
			...
			for (seqtype = SEQ_TYPE_STANDARD; seqtype < SEQ_TYPE_TOTAL; seqtype++)
			{
				...
				const input_seq *s = input_type_seq(machine, typedesc->type, typedesc->player, seqtype);
				//DAV HACK
				item->seq = *s;
				/*
				for(int i=0; i<8;i++)
	...
	else if (event != NULL && event->itemref != NULL)
	{
		...
		switch (event->iptkey)
		{
			...
			case IPT_UI_SELECT:
				...
				/*
				for(int i=0; i<8;i++)
					menustate->starting_seq.code[i] =  item->seq.code[i];
                */
...
static void menu_speed_hacks(running_machine *machine, ui_menu *menu, void *parameter, void *state)
{
	...
	if (!ui_menu_populated(menu))
	{
		ui_menu_item_append(menu, "Revert", NULL, 0, (void *)1);
		ui_menu_item_append(menu, "Safe", NULL, 0, (void *)2);
		ui_menu_item_append(menu, "Normal", NULL, 0, (void *)3);
		ui_menu_item_append(menu, "High", NULL, 0, (void *)4);
		ui_menu_item_append(menu, "Aggressive", NULL, 0, (void *)5);
		ui_menu_item_append(menu, "Very Aggressive (CPS3)", NULL, 0, (void *)6);
		ui_menu_item_append(menu, "Very Very Aggressive (CPS3)", NULL, 0, (void *)7);
	}
	...
	if (event != NULL && event->iptkey == IPT_UI_SELECT)
	{
		if((FPTR)event->itemref==1)
			...
			value = 0.80;
		else if ((FPTR)event->itemref==3)
			value = 0.65;
		else if ((FPTR)event->itemref==4)
			value = 0.50;
		else if ((FPTR)event->itemref==5)
			value = 0.33;
		else if ((FPTR)event->itemref==6)
			value = 0.25;
		else if ((FPTR)event->itemref==7)
			value = 0.20;
	}
	...
	if(value!=0.0)
	{
		...
		{
			device_t *cpu = (device_t *)&exec->device();
			cpu->set_my_clock_scale( cpu->get_clock_scale_original() * value);
		}
		if(value==1)
			popmessage("Done!");
		else
		    popmessage("Done. Warning: Speed hacks can slow down or hang the current game, but what the hell :) give it a try!");
	}
}
...
//DAV HACK
	if(menu->selected != myosd_last_game_selected)
	{
		myosd_last_game_selected = menu->selected;
	}
//DAV HACK
}
...
static int CLIB_DECL menu_select_game_driver_compare(const void *elem1, const void *elem2)
{
	...
	while (*driver1 == *driver2 && *driver1 != 0 && *driver2 != 0)
		driver1++, driver2++;

	return *driver1 - *driver2;
}

static int CLIB_DECL menu_select_game_driver_compare_2(const void *elem1, const void *elem2)
{
	const game_driver **driver1_ptr = (const game_driver **)elem1;
	const game_driver **driver2_ptr = (const game_driver **)elem2;

	const char *driver1 = (*driver1_ptr)->description;
	const char *driver2 = (*driver2_ptr)->description;
